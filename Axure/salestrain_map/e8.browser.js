...
(function (jQuery) {
	jQuery.fn.e8Browser = function (options) {
		...
		if(!browserOnClick){
			...
			browserOnClick += ",{name:'"+name+"',hasInput:"+hasInput+",zDialog:"+zDialog+",needHidden:"+needHidden;
			if(!!idKey){
		...
		if(options.hasBrowser==true){//显示浏览按钮
			...
			browser+=">";
			if(isMustInput=="2" && browserValue==""){
				//browser+=" <span class='e8_spanFloat'><img align='absmiddle' src='/images/BacoError.gif'></span>";
			}else{
				if(!!browserValue){
					...
					if(!nameSplitFlag){
						if(browserSpanValue.indexOf("__")!=-1){
							...
							browserSpanValues = browserSpanValue.split("&nbsp;");
						}else if(browserSpanValue.indexOf("&nbsp")!=-1){
							browserSpanValues = browserSpanValue.split("&nbsp");
					...
					for(var i=0;i<browserSpanValues.length && i<browserValues.length;i++){
						...
						if(!!browserSpanValue){
							...
							if((hasInput==true||hasInput=="true") && isMustInput && isMustInput!="0"){
								browser+=("<span id='")
								+(_browserValue)+("' class='e8_delClass' onclick=\"del(event,this,")+(isMustInput)+(",")+(needHidden)+","+(sb)+(");\">x</span>");
							}
							browser+="</span>";
						}
					}
		...
		if((hasInput==true||hasInput=="true") && isMustInput!="0"){
			input+="<input onblur=\"setAutocompleteOff(this);\" type= 'text' value='"+inputValue+"' issingle='"+isSingle+"' ";
			if(isSingle=="false"||isSingle==false){
			...
			input+=(" onkeydown=\"delByBS(event,'")+(inputNameAndId)+("',")+(isMustInput)+","+needHidden+","+(sb)+(");\" ");
